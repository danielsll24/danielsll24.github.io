...
ags<-mapamex %>% filter(region<=01011)
ggplot(ags, aes(long, lat, group=group)) +
    geom_polygon(fill = "white", color = "black", size = .2) +
    coord_map()+
    theme_void()+
    labs(title="Aguascalientes",subtitle = "11 Ayuntamientos")
```
#Baja California
...
ggplot(bc, aes(long, lat, group=group))+
...
ggplot(pue, aes(long, lat, group=group))+
